...
/*   Updated: 2019/02/02 13:50:42 by akupriia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
uint32_t		*sha224_word(const char *word, t_md5sha *sha224)
{
	...
	sha224->len_bytes = calc_bytenum(word, (size_t)(g_ssl->fsize + 9), 256);
	message = (uint8_t *)ft_memalloc(sha224->len_bytes);
	ft_bzero(message, sha224->len_bytes);
	ft_memcpy(message, word, g_ssl->fsize);
	sha224->len_bytes = append_pad_bits_sha((uint32_t *)message);
